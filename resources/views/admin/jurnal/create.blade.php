<x-app-layout>
    <x-slot:title>Tambah Jurnal Harian</x-slot:title>
    ...
    <x-slot:header>
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-[14px] lg:text-[15px] font-bold text-white">Tambah Jurnal Harian</h2>
                <p class="text-[11px] hidden sm:block" style="color: var(--text-muted);">Buat jurnal harian PKL baru</p>
            </div>
        </div>
    </x-slot:header>
    ...
    <form action="{{ route('admin.jurnal.store') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl mx-auto space-y-4">

        @csrf

        {{-- Data Jurnal --}}
        <div class="card p-5 anim anim-d1" style="border-color: rgba(79,142,255,0.12);">
            <div class="flex items-center gap-3 mb-5 pb-4" style="border-bottom: 1px solid rgba(79,142,255,0.1);">
                <div class="flex h-9 w-9 items-center justify-center rounded-lg" style="background: rgba(79,142,255,0.1);">
                    <i data-lucide="file-plus" class="h-[18px] w-[18px]" style="color: var(--accent-blue);"></i>
                </div>
                <div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="kegiatan" rows="4" required
                              ...
                              placeholder="Tuliskan kegiatan yang dilakukan...">{{ old('kegiatan') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="card p-5 mb-5 anim anim-d2" style="border-color: rgba(34,197,94,0.12);">
            <div class="flex items-center gap-3 mb-5 pb-4" style="border-bottom: 1px solid rgba(34,197,94,0.1);">
                <div class="flex h-9 w-9 items-center justify-center rounded-lg" style="background: rgba(34,197,94,0.1);">
                    <i data-lucide="clipboard-list" class="h-[18px] w-[18px]" style="color: var(--accent-green);"></i>
                </div>
                <div>
                ...
                <span class="ml-auto text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded" style="background: rgba(34,197,94,0.08); color: var(--accent-green);">Langkah 2</span>
            </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="kon" rows="3"
                              class="input-dark w-full rounded-lg px-3.5 py-2.5 text-[12.5px] resize-none"
                              placeholder="Kendala yang dihadapi (opsional)">{{ old('kon') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="solusi" rows="3"
                              class="input-dark w-full rounded-lg px-3.5 py-2.5 text-[12.5px] resize-none"
                              placeholder="Solusi atas kendala (opsional)">{{ old('solusi') }}</textarea>
                </div>

                <div>
                    <label class="block text-[11px] font-semibold mb-1.5" style="color: var(--text-secondary);">Foto Kegiatan</label>
                    <input type="file" name="foto" accept="image/*"
                           class="input-dark w-full rounded-lg px-3.5 py-2 text-[12px] file:mr-3 file:py-1 file:px-2.5 file:rounded-md file:text-[10px] file:font-semibold file:transition"
                           style="color: var(--text-secondary);">
                </div>

                <div class="flex items-start gap-2.5 p-3 rounded-lg" style="background: rgba(34,197,94,0.06); border: 1px solid rgba(34,197,94,0.1);">
                    <i data-lucide="info" class="h-3.5 w-3.5 shrink-0 mt-0.5" style="color: var(--accent-green);"></i>
                    <p class="text-[10.5px] leading-relaxed" style="color: var(--text-muted);">
                        Jurnal baru akan otomatis berstatus "Menunggu Review". Kendala, solusi, dan foto bersifat opsional.
                    </p>
                </div>
            </div>
        </div>
